...
abstract class YouTubeExtractor(private val context: Context?) : AsyncTask<String, Void, SparseArray<YtFile>>() {
    ...
    @Volatile private var decipheredSignature: String? = null

    private val lock = ReentrantLock()
    private val jsExecuting = lock.newCondition()

    private// Some videos are using a ciphered signature we need to get the
    ...
    val streamUrls: SparseArray<YtFile>?
        ...
        get() {
            ...
            streams = streamMap!!.split(",|url_encoded_fmt_stream_map|&adaptive_fmts=".toRegex()).dropLastWhile({ it.isEmpty() }).toTypedArray()
            val ytFiles = SparseArray<YtFile>()
            for (encStream in streams) {
                var mutableEncStream = encStream + ","
                if (!mutableEncStream.contains("itag%3D")) {
                    continue
                }
                val stream: String
                stream = URLDecoder.decode(mutableEncStream, "UTF-8")
                ...
                mat = patUrl.matcher(mutableEncStream)
                var url: String? = null
                if (mat.find()) {
                    url = mat.group(1)
                }
    ...
    private fun decipherSignature(encSignatures: SparseArray<String>): Boolean {
        ...
        if (decipherFunctionName == null || decipherFunctions == null) {
            ...
            var reader: BufferedReader? = null
            var javascriptFile: String? = null
